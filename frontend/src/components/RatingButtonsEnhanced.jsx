const RatingButton = ({ rating, label, interval, disabled, onRate, variant }) => {
  const variants = {
    again: {
  ...
  return (
    ...
    >
      <span className={`text-sm font-bold text-content-muted ${v.hoverText} uppercase tracking-widest transition-colors`}>
        {label}
      </span>
      <span className={`text-[10px] text-content-muted/50 ${v.hoverTextSub} font-mono mt-1 transition-colors border-t border-transparent ${v.hoverBorderTop} pt-1`}>
        {interval}
      </span>
      <div className={`absolute inset-x-0 bottom-0 h-0.5 bg-transparent ${v.hoverBg} transition-colors`}></div>
    </button>
  );
};

export const RatingButtonsEnhanced = ({ onRate, disabled = false, intervals }) => {
  return (
