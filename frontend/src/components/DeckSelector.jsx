  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 text-sm text-content hover:text-white transition-colors"
      >
        <span>{currentDeckName || 'loading...'}</span>
        <span className="material-symbols-outlined text-[16px] text-content-muted">
            ...
            {loading ? (
              ...
              decks.map((deck) => (
                <button
                  ...
                  className={`w-full px-4 py-2 text-left text-sm hover:bg-surface-subtle transition-colors flex items-center justify-between ${
                    deck.id === currentDeckId ? 'bg-surface-subtle text-primary' : 'text-content'
                  }`}
                >
                  <span>{deck.name}</span>
